                         ...
                         int (*acumular)(void **, unsigned *, const void *, unsigned))
{
    ...
    while(*p)
    {
        ...
        {
            if(acumular)
                acumular(&((*p)->info), &((*p)->tamInfo), d, tam);
            return CLA_DUP;
        }
    }
                            ...
                            int (*acumular)(void **, unsigned *, const void *, unsigned))
{
    ...
    {
        if((rc = cmp(d, (*p)->info)) < 0)
            return insertarRecArbolBinBusq(&(*p)->izq, d, tam, cmp, acumular);
        if (rc > 0)
            return insertarRecArbolBinBusq(&(*p)->der, d, tam, cmp, acumular);
        if(acumular)
            acumular(&((*p)->info), &((*p)->tamInfo), d, tam);
        return CLA_DUP;
    }
